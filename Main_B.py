...
force_width = int(display_width/2) #specify width to force images to display in
force_height = int(display_height/2) #specify height to force images to display in
...
width_factor = res[0]/force_width #creat a variable defining the factor between the forced image display size and capture size. This is used to generate the mask correctly
height_factor = res[1]/force_height
#The Camera Zone------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
def capture():
    ...
    #image.save("/home/pi/Desktop/Main/New/"+tim+".jpg")
    #finish timer
...
def auto_brightness(comp,control): #define function to do brightness compensation
...
def process(name): #possible inputs "full" "empty" "calibrate_max"
    global full_score, full_pass, full_ctrl, empty_ctrl, max_score, tim, full_ctrl_candidate, empty_ctrl_candidate, candidate
    print("")
    decision_start = time.time() #start image capture timer
    tot = 0 #intialize/reset the score total counter
    diff = [None]*zone_qty #intialize/reset array of difference images
    base = [None]*zone_qty #intialize/reset array of base/local control images
    compare = [None]*zone_qty #intialize/reset array of comparison/local sample images
    result = None ##intialize/reset variabe to hold the result image
    ...
    if name == "full": #if the arg is "full"
        ...
        candidate = comp.copy()
        #candidate.show()
        #comp = Image.open(comparison_folder+"/I1.jpg") #manually overwrite image to load
    ...
    #candidate = comp #define the replacemnt image candidate as the comparison image just capture

    #find the difference in the images and apply the image mask
    for i in range(0,zone_qty): #for every mask zone
        base[i] = control.crop((x[i],y[i],x[i]+w[i],y[i]+h[i])) #grab the control of the specified area, call it base
        compare[i] = comp.crop((x[i],y[i],x[i]+w[i],y[i]+h[i])) #grab the comparison of the specified area, call it compare
        base[i], compare[i] = auto_brightness(base[i],compare[i]) #run brightness compensation in the current area
        diff[i] = ImageChops.difference(base[i], compare[i]) #find difference between images from the specified area and name it "diff" or difference
        diff[i] = ImageOps.grayscale(diff[i]) #convert the difference to black and white
        diff[i] = diff[i].filter(ImageFilter.GaussianBlur(4)) #blur the image to remove noise
        diff[i] = diff[i].point( lambda p: 255 if p > 255/sens else 0) #make all pixels below the define threshold black and all above white
        tot = tot + int(np.sum(np.array(diff[i])/255)) #convert diff to an array and find the elementwise sum, add it to tot for "total". This represents the total quantity of pixels that are different
    ...
    for i in range(0,zone_qty): #for every masked zone
        diff[i] = ImageOps.colorize(diff[i], black, color, blackpoint=0, whitepoint=255, midpoint=127) #convert black and white differenc image to black and color for more contrast when displaying
        diff[i] = ImageChops.add(diff[i],compare[i]) #overlay the now green mask on top of the comaprison region
    ...
    if name == "full": #if the name is "full"
        ...
        tim = tim[:13]+"_"+tim[14:16]+"_"+tim[17:24] #change time text format from hour:minute:second to hour_minute_second (windows filesystems dont like the ":" symbol in filenames)
        result = result.resize((force_width,force_height)) #send results to the picture widget on the main page
    ...
    if good == True: #if the image passed
        ...
        if folder_has_space == True: #if the image folder enough space
            ...
            full_ctrl_candidate.save(fail_folder+tim+"_"+name+"_raw.jpg") #save the raw image to the fail folder
...
def change_res(): #define code to change image resolution
    global res, width_factor, height_factor #define resolutiion tuple as global
    ...
    width_factor = res[0]/force_width
    height_factor = res[1]/force_height
...
def set_control(name): #define code to set the control image as the current candidate image (acceptable arguments:"full","empty") 
    ...
    if name == "full": #if name is "full"
        ...
        candidate.save(comparison_folder+"/compare_"+name+"_ctrl.jpg") #save the full control candidate
        #full_ctrl = full_ctrl_candidate #write over the current control with the candidate
        full_ctrl = candidate #write over the current control with the candidate
        full_temp = full_ctrl.resize((force_width,force_height))
        full_pic.image = full_temp
...
def refresh_mask_preview(): #define code to generate and save the mask preview for use in the mask tool and archive
    ...
    print("Zones found: "+str(zone_qty))
    x = [None]*zone_qty 
    y = [None]*zone_qty
    w = [None]*zone_qty
    h = [None]*zone_qty
    ...
    current_mask = current_mask.resize((force_width,force_height))
...
def launch_mask_tool(): #define code to launch masking tool window
    ...
    image = full_ctrl.resize((force_width,force_height))
    pid = drawing.image(0,0,image=image, width = force_width, height = force_height) #update the masking window image with the new control image and svae the pic id
...
def finish(event_data): #define what to do when click is released
    ...
    msk.rectangle([(x1*width_factor,y1*height_factor),(x2*width_factor,y2*height_factor)], fill="white") #draw an identical rectangle to the PIL image
...
def kill_all(): #define code to kill all drawings
    ...
    pid = drawing.image(0,0,image=comparison_folder+"/compare_full_ctrl.jpg", width = force_width, height = force_height) #add the image back in and save pic id to pid
...
app = App(title='Main', layout='auto', width = 1920, height = 1080) #, width = display_width, height = display_height) #create the main application window in a fullsize window
#app2 = Window(app, title='Pic', layout='auto', width = 1920, height = 1080) #create the main application window in a fullsize window
...
#control preview------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
full_preview = Window(app, title="full preview", width=force_width, height=force_height+100, visible=False) #create a "full" preview window

full_temp = Image.open(comp_path+"full_ctrl.jpg")
full_temp = full_temp.resize((force_width,force_height))
full_pic = Picture(full_preview, image=full_temp) #add the current full control as an image
...
#setup main window-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#row_1 = Box(app2, width=res[0]*2, height=res[1], align='top') #create a container for the images. call it row_0
pic_full = Picture(app, image=menu_image, align='top', height=force_height, width = force_width) #create picture widget to show the mold open image
...
drawing = Drawing(mask_win,960,540) #create drawing widget that fills the window
pid = drawing.image(0,0,image=comparison_folder+"/compare_full_ctrl.jpg", width = force_width, height = force_height) #add the current control image to the drawing widget
